   ...
   if (is_uploaded_file($foto['tmp_name'])) {
      ...
      if (!in_array(($tipe),$ext)){ //cek ekstensi file
         ...
         if (move_uploaded_file($foto['tmp_name'],$dir.$newName)) {
            ...
            $sql = $con->prepare("UPDATE t_kandidatrw SET nama_calonrw = ? , fotorw = ?, visirw = ?, misirw = ? WHERE id_kandidatrw = ?") or die($con->error);
            $sql->bind_param('sssss', $nama, $newName, $visi, $misi, $id);
            $sql->execute();

            header('location:../dashboard.php?page=kandidatrw');
      ...
      $sql = $con->prepare("UPDATE t_kandidatrw SET nama_calonrw = ?, visirw = ?, misirw = ? WHERE id_kandidatrw = ?") or die($con->error);
